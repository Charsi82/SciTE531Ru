...
void TButton::state(int state)
{
	switch (state)
	{
	case BST_UNCHECKED:
		send_msg(BM_SETSTATE, BST_UNCHECKED);
		break;
	case BST_CHECKED:
		send_msg(BM_SETSTATE, BST_CHECKED);
		break;
	case BST_PUSHED:
		send_msg(BM_SETSTATE, BST_PUSHED);
		break;
	default:
		send_msg(BM_SETSTATE, BST_INDETERMINATE);
	}
}

int TButton::state() const
{
	return send_msg(BM_GETSTATE) - 520; // [off - 0, on - 1, grayed - 2]
}
...
TCheckBox::TCheckBox(TWin* parent, pchar caption, int id, bool is3state)
	: TButton(parent, caption, id, is3state ? ButtonStyle::AUTO3STATE : ButtonStyle::AUTOCHECKBOX)
{
	calc_size();
}
...
TRadioButton::TRadioButton(TWin* parent, pchar caption, int id, bool is_auto)
	:TButton(parent, caption, id, is_auto ? ButtonStyle::AUTORADIOBUTTON : ButtonStyle::RADIOBUTTON)
{
}
...
{
	ctrl->m_wnd_proc = (void*)SetWindowLongPtr((HWND)ctrl->handle(), GWLP_WNDPROC, (LONG_PTR)FormWndProc);
}
