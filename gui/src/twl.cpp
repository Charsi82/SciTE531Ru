...
{
	HWND st_wnd = CreateStatusWindow(WS_CHILD | WS_VISIBLE | SBARS_SIZEGRIP, L"", m_hwnd, -1);
...
void TWin::set_tooltip(int id, pchar tiptext)
{
	TOOLINFO ti;
	HWND hwTooltip = CreateWindow(TOOLTIPS_CLASS, L"", TTS_ALWAYSTIP,
...
TLabel::TLabel(TWin * parent, pchar text, int id, DWORD style)
//--------------------------------------
	: TControl(parent, L"STATIC", text, id, style)
{ }
...
void TProgressControl::set_barcolor(COLORREF clr)
{
	send_msg(PBM_SETBARCOLOR, 0, clr);
}

void TProgressControl::set_backcolor(COLORREF clr) 
{
	send_msg(PBM_SETBKCOLOR, 0, clr);
}
...
DLGFN DialogProc(HWND hdlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDOK:
			...
			[[fallthrough]];
		case IDCANCEL:
			if (This->modeless()) SendMessage(hdlg, WM_CLOSE, 0, 0);
