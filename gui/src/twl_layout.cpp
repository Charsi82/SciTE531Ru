...
Layout::Layout(TEventWindow* form, TEventWindow* mform)
	:m_height(0), m_ctrl_handler(NULL), m_last_ctrl(NULL), m_parent(NULL)
{
...
TControl* Layout::add(TControl* ctrl, bool new_line)
{
	if (ctrl->height() < 10)
	...
	m_width = max(m_xpos, m_width);
	_controls_[m_row][m_col++] = ctrl;
	if (new_line) next_row();
	return ctrl;
}

void Layout::next_row()
{
	m_ypos += m_last_ctrl->height() + m_yspacing;
	m_xpos = m_left;
	m_max_col = max(m_max_col, m_col);
	m_row++;
	m_col = 0;
}

void Layout::space(int yy, int xx)
{
	m_ypos += yy;
	if (xx != -1) m_xpos = m_left + xx;
}

void Layout::center_row()
{
...
void Layout::left_align(int row)
{
	int sr, er;
	...
	for (int col = 1; col < m_max_col; col++) {  // first col will already be aligned by default
		...
		for (row = sr; row < er; row++) {
			...
			m_width = max(m_width, rt.right); //*new
		}
	}
}

void Layout::add_label(pchar caption, bool new_line)
{
	if (caption && *caption) {
...
void Layout::add_ctrl(TControl* ctrl, bool new_line)
{
	if (s_field_width > 0) {
		int h = ctrl->height();
		if (h < 10) h = 20;
...
void Layout::add_data_control(TControl* ctrl, Data* obj/*, Converter* co*/, bool new_line)
{
...
void Layout::add_edit(Data* obj, int id, bool new_line)
{
	add_data_control(
		new TEdit(m_parent, L"", id),
		obj,
		//new EditConverter,
		new_line
	);
...
void Layout::add_button(pchar capt, int id, bool new_line)
{
	TButton* btn = new TButton(m_parent, capt, id);
	add_ctrl(btn, new_line);
}

void Layout::add_listbox(int id, int height, bool new_line, Data* obj)
{
...
void Layout::add_checkbox(pchar capt, Data* obj, int id, bool new_line)
{
	TCheckBox* box = new TCheckBox(m_parent, capt, id, true);
	box->calc_size();
	add_data_control( box, obj, /*new CheckboxConverter,*/ new_line );
}

void Layout::add_action(Item& item)
{
	if (item.id == -1) item.id = m_last_id++;
	m_ctrl_handler->add(item);
}

void Layout::finish_off()
{
