...
void OutputMessage(lua_State* L)
{
	if (lua_isstring(L, -1)) {
		size_t len = 0;
		gui_string tmp_str = StringFromUTF8(lua_tolstring(L, -1, &len));
		const wchar_t* msg = tmp_str.data();
		wchar_t* buff = new wchar_t[len + 2];
		wcsncpy_s(buff, len + 2, msg, len);
		buff[len] = '\n';
		buff[len + 1] = '\0';
		lua_pop(L, 1);
		if (lua_checkstack(L, 3)) {
			...
			lua_insert(L, -2);
			lua_pushstring(L, UTF8FromString(buff).data());
			lua_pcall(L, 2, 0, 0);
		}
		delete[] buff;
	}
...
bool IsInputValid(lua_State* L, const wchar_t* str, wchar_t ch, int checker)
{
	if (checker && ch) {
		...
		lua_pushstring(L, UTF8FromString(str).data());
		lua_pushlstring(L, UTF8FromString(&ch).data(), 1);
...
//------------------------------------------------------------------------------
LRESULT CALLBACK InputBox::EditHandler(HWND hctrl, UINT msg, WPARAM wParam, LPARAM lParam)
{
...
//------------------------------------------------------------------------------
LRESULT CALLBACK InputBox::DlgHandler(HWND hdlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	if (msg == WM_INITDIALOG) {
		...
		originalEditHandler = (WNDPROC)SetWindowLongPtr(hedit, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(EditHandler));
		...
		// Помещаем текст в лейбл приглашения
		SetDlgItemText(hdlg, IDC_PROMPTTEXT, self->prompt);
		...
		case WM_COMMAND:
		{
			...
			case IDOK:
				...
				if (!IsInputValid(self->luaState, self->editText, L'\0', self->onChar)) {
					return TRUE;
				}
				[[fallthrough]];
			case IDCANCEL:
		...
		case WM_GETICON:
			...
			case ICON_SMALL2:
				...
				SetWindowLongPtr(hdlg, DWLP_MSGRESULT, reinterpret_cast<LONG_PTR>(self->smallIcon));
				break;
			default:
				return FALSE;
			}
...
HWND FindScite()
{
	...
	//return SciTE;
	return FindWindow(L"SciTEWindow", NULL);
}

//------------------------------------------------------------------------------
int InputBox::ShowModal()
{
	int result = DialogBoxParam(GetModuleHandle(L"shell.dll"), L"IBOX_DLG", FindScite(), (DLGPROC)&DlgHandler, reinterpret_cast<LPARAM>(this));
	if (result == -1) {
...
extern int showinputbox(lua_State* L)
{
	...
	if (res)
		lua_pushstring(L, UTF8FromString(dlg.Text()).data());
	else
		lua_pushnil(L);
