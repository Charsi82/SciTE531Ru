...
bool SciTEBase::Open(const FilePath &file, OpenFlags of) {
	...
	resolveLinkFile(_file_path);
	FilePath absPath(GUI::StringFromUTF8(_file_path));
#else
	...
	if (props.GetInt("load.on.activate")) {
		...
		if ((newModTime != 0) && (newModTime != CurrentBuffer()->fileModTime)) {
			...
			GUI::gui_string msg = LocaliseMessage(
				...
				filePath.AsInternal());
			const MessageBoxChoice decision = WindowMessageBox(wSciTE, msg, MB_YESNO);
...
#ifdef RB_FRLS
				GrepRecursive(gf, fPath, searchString, fileTypes, excludedTypes, basePath); //!-change-[FindResultListStyle]
#else
...
void SciTEBase::InternalGrep(GrepFlags gf, const FilePath &directory, GUI::gui_string_view fileTypes, GUI::gui_string_view excludedTypes,
	...
	if (!FlagIsSet(gf, GrepFlags::stdOut)) {
		...
		//!-start-[FindResultListStyle]
		if (props.GetInt("lexer.errorlist.findliststyle", 1)) {
			std::string dir = directory.AsUTF8();
...
#ifdef RB_FRLS
	GrepRecursive(gf, directory, searchString.c_str(), fileTypes, excludedTypes, basePathLen); //!-change-[FindResultListStyle]
#else
